...
- Add Beyond the Code entries through `/manage/personal`. All eleven categories begin empty; the public `/api/personal` endpoint returns only visible records, optionally filtered by `?category=hobbies`. Card size and numeric order are controlled per entry in the CMS. No personal content is seeded into the configured database.
...
- For browser QA without a persistent database, set a disposable `UI_TEST_PASSWORD` and run `npm run test:ui-server --prefix server`. This test-only process starts an in-memory database with clearly labeled demo personal cards on port 3000 (or `UI_TEST_PORT`); stopping it removes the data.

The public pages display terminal-style loading, empty, and retry states when API data is unavailable. The control panel handles profile, projects, skills, experience, certificates, Beyond the Code, appearance settings, and contact messages. Its forms call the protected API directly; refreshing the public page reflects saved records.
